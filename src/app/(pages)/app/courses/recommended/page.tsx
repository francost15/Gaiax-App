...
const categories = ['Todos', ...Array.from(new Set(RECOMMENDED_COURSES.map(course => course.category)))]
...
export default function RecommendedCoursesPage() {
  ...
  const filteredCourses = useMemo(() => {
    return RECOMMENDED_COURSES.filter(course => 
  ...
  return (
    <div className="container px-4 py-8 mx-auto text-gray-100 ">
      <div className="flex items-center justify-between mb-4 sm:mb-6">
        <h1 className="text-xl font-bold text-black dark:text-white sm:text-2xl">Cursos Recomendados</h1>
        <Link href="/app" passHref>
          <Button 
            name='Volver al inicio'
            variant="outline" 
            className="flex items-center gap-2 px-2 py-1 text-xs text-white transition-all duration-300 ease-in-out transform rounded-2xl bg-primaryper border-primaryper hover:bg-primary-hover hover:border-primary-hover hover:text-white hover:shadow-lg hover:scale-105 sm:text-sm sm:px-4 sm:py-2"
          >
...
{/* courses cards */}
      <div className="grid grid-cols-1 gap-4 sm:grid-cols-2 lg:grid-cols-3 xl:grid-cols-4">
        {filteredCourses.map((course, index) => (
          <Card key={index} className="overflow-hidden text-gray-100 transition-all duration-300 bg-white border rounded-xl border-neutral-300 dark:border-neutral-700 hover:shadow-lg dark:bg-neutral-900 dark:hover:bg-neutral-800 hover:bg-neutral-50 hover:scale-105">
            <CardContent className="p-4">
              <div className="flex items-start justify-between mb-2">
                <h3 className="text-base font-semibold text-black dark:text-gray-100 line-clamp-1">{course.title}</h3>
                <Badge 
                  variant="outline" 
              ...
              <p className="mb-3 text-xs text-neutral-700 dark:text-gray-400 line-clamp-2">{course.description}</p>
              <div className="flex items-center justify-between mb-3">
                <Badge variant="secondary" className="flex items-center gap-1 text-xs text-neutral-700 dark:text-gray-400">
